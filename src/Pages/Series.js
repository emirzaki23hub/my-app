...
function Series() {
  ...
  const [filterStatus, setFilterStatus] = useState("airing_today");
  ...
  const fetchMovies = async (type, page, filterStatus) => {
    try {
      ...
      const res = await axios.get(
        `https://api.themoviedb.org/3/tv/${type}?api_key=c75d96c32b97c8e7dd7ef5d195680227&&page=${page}`
      );
      setData(res.data);
    } catch (error) {
      console.log(error);
